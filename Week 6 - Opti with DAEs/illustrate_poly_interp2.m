...
xi_fine = []; phi = {phi1, phi2, phi3, phi4};
for j = 1:4
    xi_fine(:,j) = interp1(0:n, xi(:,j), ...
        linspace(0,n,300), 'cubic'); %#ok
end
filename = 'poly_interp2.gif'; f = figure;
for j = 1:height(xi_fine)

    % Interpolation of x(t)

    for k = 1:4
        xt = xi_fine(j,k)*phi{k}(t);
        subplot(4,1,k);
        plot(t, xt, 'k', 'LineWidth', 1.2); 
        hold on; set(gcf, 'Color','w');
        scatter(tm(k), xi_fine(j,k), 'm', 'filled'); 
        hold off; grid on; ylim([-5 5]); 
    end
